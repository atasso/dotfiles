...
function telescopePickers.prettyMarksPicker(localOptions)
  if localOptions ~= nil and type(localOptions) ~= 'table' then
    print("Options must be a table.")
    return
  end
  ...
  local originalEntryMaker = telescopeMakeEntryModule.gen_from_marks(options)
  ...
  options.entry_maker = function(line)
    ...
    originalEntryTable.display = function(entry)
      ...
      -- Split the string based on spaces and backslashes
      for substring in entry.ordinal:gmatch("%S+") do
        table.insert(substrings, substring)
      end
      local tail, path = telescopePickers.getPathAndTail(entry.filename)
      local tailForDisplay = tail .. ' '
      local icon, iconHighlight = telescopeUtilities.get_devicons(tail)
      local mark = substrings[1]
      local lineMark = substrings[2]

      return displayer {
        { icon, iconHighlight },
        tailForDisplay,
        {  mark .. ' (' .. lineMark .. ')', "TelescopeResultsNumber" },
        { path, "TelescopeResultsComment" },
      }
    end
  ...
  require('telescope.builtin').marks(options)
end

function telescopePickers.prettyReferencePicker(localOptions)
  if localOptions ~= nil and type(localOptions) ~= 'table' then
    print("Options must be a table.")
    return
  end
